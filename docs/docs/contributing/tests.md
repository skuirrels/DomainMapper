---
sidebar_position: 2
description: How DomainMap is tested and linted.
---

# Tests and linting

DomainMap is continuously tested by GitHub Actions.
...
The unit tests are located in the `tests/DomainMap.*.Tests` projects.
...
The integration tests are located in `tests/DomainMap.IntegrationTests`.
...
See also the [debugging documentation](./debugging.md) topic on how to debug DomainMap.
...
To work with the tests of DomainMap you may find it helpful to read the documentation of it.
For several IDE's, nice plugins make working with VerifyTests easier.

## Linting

The source of DomainMap is linted with multiple dotnet analyzers.
