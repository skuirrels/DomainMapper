---
sidebar_position: 3
description: How to debug DomainMap.
---
...
To debug the code generated by DomainMap just set a breakpoint in the generated code and you are ready to go.
Check [here](../configuration/generated-source.mdx) on how to inspect the generated code.

## Unit tests

To debug the DomainMap in unit tests, set a breakpoint
in the code of DomainMap which you want to debug and run the tests in debug mode.

## Integration tests

If your IDE supports the `DebugRoslynComponent` launch configuration command,
you can set your breakpoints and debug the preconfigured `IntegrationTests` profile which will debug DomainMap
...
DomainMap uses a compile constant `DEBUG_SOURCE_GENERATOR` to determine whether to attach a debugger.
...
and the generated shell scripts need to be on the path (DomainMap calls `rider attach-to-process {pid} {DomainMap-Sln-File}`).

## CSC warning CS8785 stack trace

If you see the compiler warning `CS8785: Generator 'DomainMapGenerator' failed to generate source`,
