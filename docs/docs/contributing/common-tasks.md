---
sidebar_position: 5
description: Step by step guides for common tasks one may encounter when contributing to DomainMap.
---

# Common tasks

This page provides todo lists for some common tasks one may encounter while contributing to DomainMap.

## New diagnostic

To introduce a new diagnostic follow these steps:

1. Ensure no existing diagnostic in `DomainMap.Diagnostics.DiagnosticDescriptors.cs` matches the new use case.
...
3. Add the new diagnostic to `AnalyzerReleases.Shipped.md` (DomainMap does not use the `Unshipped` file).
...
If new public API surface is introduced in `DomainMap.Abstractions` or existing API surface is modified,
...
2. Create a new file `DomainMap.Roslyn$(Version).props` in `src/DomainMap` similar to the existing ones
   and define constants and include dependencies as needed.
3. Update the default `ROSLYN_VERSION` in `src/DomainMap/DomainMap.csproj`.
...
7. Add the new version in `DomainMap.IntegrationTests.Helpers.Versions` and `DomainMap.IntegrationTests.BaseMapperTest.GetCurrentVersion`.
...
DomainMap Mappings use Roslyn syntax trees.
...
The `DomainMap.Emit.Syntax.SyntaxFactoryHelper` and `Microsoft.CodeAnalysis.CSharp.SyntaxFactory` classes help building these syntax trees.
