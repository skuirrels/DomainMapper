---
sidebar_position: -3
description: How to upgrade to DomainMap v3.0 and a list of all its breaking changes
---
...
[![NuGet v3.0.0](https://img.shields.io/badge/NuGet-v3.0-blue?style=flat-square)](https://www.nuget.org/packages/DomainMap/3.0.0)
...
DomainMap versions < 3.0 considered all members which are either internal in the same assembly as the mapper or public.
Since v3.0 DomainMap considers all members which are accessible by the mapper.
...
All attributes in `DomainMap.Abstractions` are now sealed.
