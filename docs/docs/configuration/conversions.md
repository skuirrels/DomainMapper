---
sidebar_position: 20
description: A list of conversions supported by DomainMap
---

# Conversions

DomainMap implements several types of automatic conversions (in order of priority):
...
| Parse method         | Uses a static `Parse` method on the target type                                                                                                                                                          | Source type is a `string` and target has a static method with the following signature: `TTarget Parse(string)` or `TTarget Parse(string, IFormatProvider)`.                                                                                                                 |
...
This only works when the annotated method is in the same assembly as the mapper unless DomainMap attributes are preserved at runtime; see the ["Ignore a member at definition"](./mapper.mdx#ignore-a-member-at-definition) section in the mapper configuration docs.

## Disable all automatic conversions

To disable all conversions supported by DomainMap set `EnabledConversions` to `None`:

```csharp
// highlight-start
[DomainMapper(EnabledConversions = MappingConversionType.None)]
// highlight-end
...
// highlight-start
[DomainMapper(EnabledConversions = MappingConversionType.All & ~MappingConversionType.ToStringMethod)]
// highlight-end
public partial class CarMapper
{
  ...
...
// highlight-start
[DomainMapper(EnabledConversions = MappingConversionType.Constructor | MappingConversionType.ExplicitCast)]
// highlight-end
public partial class CarMapper
{
  ...
