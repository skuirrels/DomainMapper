...
When DomainMap resolves a nested type mapping (e.g., `A.Nested → B.Nested`),
...
DomainMap will **not** auto-generate mapping methods with additional parameters.
...
<Tabs>
  <TabItem value="nested-declaration" label="Declaration" default>
    ...
    public partial class OrderMapper
    {
      ...
      // highlight-start
      // DomainMap will call MapItem with the currentUserId parameter forwarded
...
- A mapping with additional mapping parameters cannot be the default mapping
  (it is not used by DomainMap when encountering a nested mapping for the given types),
