...
DomainMap supports static mappers and extension methods:
...
Static methods are supported in non-static mapper classes. This supports the static interface use case. When a static mapping method is present, to simplify mapping method resolution and reduce confusion about which mapping method DomainMap uses, all methods must be static.
