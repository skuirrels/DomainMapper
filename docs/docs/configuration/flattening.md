...
DomainMap tries to figure out flattenings automatically by making use of the PascalCase C# notation.
If DomainMap can't resolve the target or source property correctly, it is possible to manually configure it by applying the `MapPropertyAttribute`
...
:::info
Unflattening is not automatically configured by DomainMap and needs to be configured manually via `MapPropertyAttribute`.
:::
...
Similarly, the automatic flattening described above has precedence over the nested members:
...
DomainMap supports a "fullnameof" to simplify these configurations, see [full-nameof](./full-nameof.md).
