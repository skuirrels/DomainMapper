...
If no parameterless constructor is available, DomainMap will try to map to the constructor arguments (see [constructor mappings](./ctor-mappings.md)).
...
and set the `DomainMap.Abstractions.ObjectFactoryAttribute` attribute.
...
The first object factory with a matching signature is used to construct the desired type by DomainMap.
...
DomainMap also supports generic object factories with or without type constraints.
...
DomainMap supports several object factory method signatures.
