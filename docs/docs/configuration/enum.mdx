...
It is also possible to set the strategy for the entire mapper via the `DomainMapperAttribute`.
...
<Tabs>
  ...
  ```csharp
  // highlight-start
  [DomainMapper(EnumMappingStrategy = EnumMappingStrategy.ByName, EnumMappingIgnoreCase = true)]
  // highlight-end
...
You can specify the naming strategy using `NamingStrategy` in `MapEnumAttribute` or `EnumNamingStrategy` in `DomainMapperAttribute` and `MapperDefaultsAttribute`.
...
<Tabs>
    <TabItem value="global" label="Global (mapper level)" default>
        ...
        ```csharp
        // highlight-start
        [DomainMapper(EnumNamingStrategy = EnumNamingStrategy.SnakeCase)]
        // highlight-end
        public partial class CarMapper
        {
            ...
...
This is especially useful when applying [strict enum mappings](#one-side-strict-enum-member-mappings).
...
By default DomainMap emits diagnostics with a severity of warning if there are unmapped source or target enum members.
...
<Tabs>
    <TabItem value="global" label="Global (mapper level)" default>
        ...
        ```csharp
        // highlight-start
        [DomainMapper(RequiredEnumMappingStrategy = RequiredMappingStrategy.Source)]
        // highlight-end
        public partial class CarMapper
        {
            ...
