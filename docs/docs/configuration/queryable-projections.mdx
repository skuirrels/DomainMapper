...
DomainMap does support `IQueryable<T>` projections:
...
DomainMap tries to inline user-implemented mapping methods.
...
If DomainMap is unable to inline a user-implemented method, `RMG068` is reported.
...
Otherwise, DomainMap cannot resolve the user-implemented mapping methods.
...
{
    // highlight-start
    public static partial IQueryable<CarDto> ProjectToDto(this IQueryable<Car> q, int currentUserId);
    // highlight-end
}
```
...
In addition to `IQueryable<T>` projections, DomainMap also supports generating `Expression<Func<TSource, TTarget>>` directly.
