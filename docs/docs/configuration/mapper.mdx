---
sidebar_position: 0
description: Define a mapper with DomainMap.
---
...
The `DomainMapperAttribute` provides options to customize the generated mapper class.
...
DomainMap configurations can also be set via MSBuild properties.
This allows to configure defaults in the project file (e.g. `.csproj`) or `Directory.Build.props`.
Property names correspond to the `DomainMapperAttribute` properties prefixed with `DomainMap`.

```xml
<PropertyGroup>
  <DomainMapEnumMappingIgnoreCase>true</DomainMapEnumMappingIgnoreCase>
  <DomainMapRequiredMappingStrategy>Both</DomainMapRequiredMappingStrategy>
</PropertyGroup>
```
...
By default, DomainMap does not create deep copies of objects to improve performance.
...
To create deep copies, set the `UseDeepCloning` property on the `DomainMapperAttribute` to `true`.
...
When deep cloning a `Stack<T>`, the order of elements is preserved by default.
This behavior can be configured via the `StackCloningStrategy` property on the `DomainMapperAttribute`.

- `StackCloningStrategy.PreserveOrder` (default): Preserves the order of elements.
...
that project needs to preserve the DomainMap attributes at runtime,
see [preserving DomainMap attributes at runtime](../getting-started/installation.mdx#preserving-the-attributes-at-runtime).
Without this, the `MapperIgnoreAttribute` is not visible to DomainMap.

#### Ignore obsolete members

By default, DomainMap will map source/target members marked with `ObsoleteAttribute`.
This can be changed by setting the `IgnoreObsoleteMembersStrategy` of a method with `MapperIgnoreObsoleteMembersAttribute`,
or by setting the `IgnoreObsoleteMembersStrategy` option of the `DomainMapperAttribute`.
...
<Tabs>
    <TabItem value="global" label="Global (mapper level)" default>
        ...
        ```csharp
        // highlight-start
        [DomainMapper(IgnoreObsoleteMembersStrategy = IgnoreObsoleteMembersStrategy.Both)]
        // highlight-end
        public partial class CarMapper
        {
            ...
    ...
    <TabItem value="method" label="Local (mapping method level)">
        ...
        otherwise the `DomainMapperAttribute` property `IgnoreObsoleteMembersStrategy` will be used.
...
If properties/fields differ in naming conventions, the `DomainMapperAttribute` can be used with the `PropertyNameMappingStrategy` option.
...
DomainMap allows the customization of how `null` values are handled when mapping properties.
...
DomainMap tries to return a default value.
...
By default DomainMap emits diagnostics with a severity of warning if there are unmapped source or target members.
...
To customize the format provider / culture to be used by DomainMap when calling `ToString` or `Parse`
format providers can be used.
A format provider can be provided to DomainMap by simply annotating a field or property within the Mapper with the `FormatProviderAttribute`.
...
By default, DomainMap uses the method name as the identifier for each mapping.
