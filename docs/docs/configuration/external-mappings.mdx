...
## External mapping with UseDomainMapper / UseStaticDomainMapper

DomainMap can also consider mappings implemented in other classes.
In order for DomainMap to find the mappings, they must be made known with `UseDomainMapper` / `UseStaticDomainMapper`.

<Tabs>
    <TabItem value="static" label="Static">

        For static mappings, `UseStaticDomainMapper` can be used:

        ```csharp
        [DomainMapper]
        // highlight-start
        [UseStaticDomainMapper(typeof(BananaMapper))]
        // highlight-end
    ...
    <TabItem value="instance" label="Instance">
        ...
        To use the mappings of an object instance `UseDomainMapper` can be used:
        ...
        :::info
        The initialization of fields and properties annotated with `UseDomainMapper` needs to be done by the user.
        :::
...
Whenever DomainMap needs a mapping from `BananaBox` to `BananaBoxDto` inside the `BoxMapper` implementation,
it will use the provided implementation by the `BananaMapper`.

Used mappers themselves can be DomainMap backed classes.  
...
It is also possible to configure DomainMap to always consider certain external mappers
without needing to specify them on each mapper class.  
This can be done through the `UseStaticDomainMapper` attribute.

```csharp
[assembly: UseStaticDomainMapper(typeof(BananaMapper))]
[assembly: UseStaticDomainMapper(typeof(AppleMapper))]
```

This makes the static mapping methods of the specified mapper classes available to all mappers in the assembly.

## External method reference

### Static external method reference

DomainMap also supports referencing methods directly from an external class. To do this, prefix the path
