...
/// <summary>
/// Marks a partial class as a compile-time domain boundary mapper.
/// DomainMap generates structural mapping code, while constructors, domain factories,
/// and user mapping methods remain responsible for business invariants.
/// </summary>
[AttributeUsage(AttributeTargets.Class)]
[Conditional("DOMAINMAP_ABSTRACTIONS_SCOPE_RUNTIME")]
public class DomainMapperAttribute : Attribute
{
    /// <summary>
    ...
    /// <summary>
    /// Enabled conversions which DomainMap automatically implements.
    ...
    /// <summary>
    /// Determines the access level of members that DomainMap will map.
    /// </summary>
    public MemberVisibility IncludedMembers { get; set; } = MemberVisibility.AllAccessible;

    /// <summary>
    /// Determines the access level of constructors that DomainMap will take into account.
    /// </summary>
    ...
    ///
    /// To discover mappings in external mappers (<seealso cref="UseDomainMapperAttribute"/> and <seealso cref="UseStaticDomainMapperAttribute"/>)
    /// the same rules are applied:
    /// If set to <c>true</c> all methods with a mapping method signature are automatically discovered.
    /// If set to <c>false</c> methods with a <see cref="UserMappingAttribute"/> and if the containing class has a <see cref="DomainMapperAttribute"/>
