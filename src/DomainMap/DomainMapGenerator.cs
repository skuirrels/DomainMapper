...
[Generator]
public class DomainMapGenerator : IIncrementalGenerator
{
    public static readonly string DomainMapperAttributeName = typeof(DomainMapperAttribute).FullName!;
    public static readonly string MapperDefaultsAttributeName = typeof(MapperDefaultsAttribute).FullName!;
    public static readonly string UseStaticDomainMapperName = typeof(UseStaticDomainMapperAttribute).FullName!;
    public static readonly string UseStaticDomainMapperGenericName = typeof(UseStaticDomainMapperAttribute<>).FullName!;
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var compilationContext = context
            ...
            .WithTrackingName(DomainMapGeneratorStepNames.BuildCompilationContext);
        ...
        var mapperDefaultsAndDiagnostics = compilationContext
            ...
            .WithTrackingName(DomainMapGeneratorStepNames.BuildMapperDefaults);
        ...
        var useStaticMappers = SyntaxProvider
            .GetUseStaticDomainMapperDeclarations(context)
            .Select(BuildStaticMappers)
            .WithTrackingName(DomainMapGeneratorStepNames.BuildUseStaticDomainMappers);
        ...
        var diagnostics = mappersAndDiagnostics
            ...
            .WithTrackingName(DomainMapGeneratorStepNames.ReportDiagnostics);
        context.ReportDiagnostics(diagnostics);

        // output the mappers
        var mappers = mappersAndDiagnostics.Select(static (x, _) => x.Mapper).WithTrackingName(DomainMapGeneratorStepNames.BuildMappers);
    ...
    private static MapperAndDiagnostics? BuildDescriptor(
        ...
        ImmutableArray<UseStaticDomainMapperConfiguration> assemblyScopedStaticMappers,
    ...
    )
    {
        ...
        var mapperConfiguration = attributeDataAccessor.AccessFirstOrDefault<DomainMapperAttribute, MapperConfiguration>(
    ...
    private static ImmutableArray<UseStaticDomainMapperConfiguration> MapStaticMappers(
        ImmutableArray<UseStaticDomainMapperConfiguration> staticMappers,
        CSharpCompilation compilation
    )
    {
        ...
        var staticMappersBuilder = ImmutableArray.CreateBuilder<UseStaticDomainMapperConfiguration>(staticMappers.Length);
        foreach (var config in staticMappers)
        {
            ...
            staticMappersBuilder.Add(new UseStaticDomainMapperConfiguration(mappedMapperType));
        }
    ...
    private static ImmutableArray<UseStaticDomainMapperConfiguration> BuildStaticMappers(
    ...
    )
    {
        ...
        {
            return ImmutableArray<UseStaticDomainMapperConfiguration>.Empty;
        }

        var configurations = ImmutableArray.CreateBuilder<UseStaticDomainMapperConfiguration>(attributeDataList.Length);
        foreach (var attributeData in attributeDataList)
        {
            ct.ThrowIfCancellationRequested();

            var config = AttributeDataAccessor.Access<UseStaticDomainMapperAttribute<object>, UseStaticDomainMapperConfiguration>(
                attributeData
            );
