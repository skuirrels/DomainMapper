...
{
    private const string DomainMapClassName = "DomainMapBenchmarkMapper";
    ...
    public static ComparisonCodeParityReport Evaluate(
        string domainMapGeneratedSource,
        string mapperlyGeneratedSource,
        string mapperDeclarationsSource
    )
    {
        var domainMap = MapperModel.Create(DomainMapClassName, domainMapGeneratedSource, mapperDeclarationsSource);
        ...
        var scenarios = _scenarioRoots
            .Select(x => BuildEntry(x.Key, x.Value, domainMap, mapperly))
            .OrderBy(x => x.Scenario, StringComparer.Ordinal)
            .ToArray();
        return new ComparisonCodeParityReport(scenarios);
    }

    public static void Write(string domainMapGeneratedPath, string mapperlyGeneratedPath, string mapperDeclarationsPath, string outputPath)
    {
        var report = Evaluate(
            File.ReadAllText(domainMapGeneratedPath),
    ...
    private static ComparisonCodeParityEntry BuildEntry(string scenario, string rootMethod, MapperModel domainMap, MapperModel mapperly)
    {
        var domainMapCanonical = domainMap.Canonicalize(rootMethod);
        var mapperlyCanonical = mapperly.Canonicalize(rootMethod);
        var domainMapFingerprint = Fingerprint(domainMapCanonical);
        var mapperlyFingerprint = Fingerprint(mapperlyCanonical);
        return new ComparisonCodeParityEntry(
            scenario,
            rootMethod,
            string.Equals(domainMapCanonical, mapperlyCanonical, StringComparison.Ordinal),
            domainMapFingerprint,
            mapperlyFingerprint
        );
    }
    ...
    private sealed class MapperModel
    {
        ...
        private static string RemoveRedundantParentheses(SyntaxNode node) =>
            new RedundantParenthesesRewriter()
                ...
                .ToFullString()
                .Replace("global::DomainMap.Benchmarks.", string.Empty, StringComparison.Ordinal);
