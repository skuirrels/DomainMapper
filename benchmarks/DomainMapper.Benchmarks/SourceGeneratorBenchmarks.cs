...
{
    private const string DomainMapperAttribute = "DomainMapper.Abstractions.DomainMapper";
    ...
    private GeneratorDriver? _domainMapperDriver;
    private Compilation? _domainMapperCompilation;
    ...
    public void Setup()
    {
        _domainMapperCompilation = BuildCompilation(DomainMapperAttribute, typeof(DomainMapperAttribute).Assembly.Location);
        ...
        _domainMapperDriver = CreateDriver(new DomainMapperGenerator().AsSourceGenerator());
        _mapperlyDriver = CreateDriver(LoadMapperlyGenerator());

        ValidateGenerator(_domainMapperDriver, _domainMapperCompilation, "DomainMapper");
    ...
    {
        var completedDriver = driver.RunGeneratorsAndUpdateCompilation(compilation, out var outputCompilation, out var diagnostics);
        var errors = diagnostics.Concat(outputCompilation.GetDiagnostics()).Where(x => x.Severity == DiagnosticSeverity.Error).ToArray();
        if (errors.Length > 0)
    ...
    [Benchmark]
    [BenchmarkCategory("ColdGenerator")]
    public object DomainMapperColdGeneration() =>
        _domainMapperDriver!.RunGeneratorsAndUpdateCompilation(_domainMapperCompilation!, out _, out _);
}
