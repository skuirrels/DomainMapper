...
public class UserMethodAdditionalParameterForwardingTest
{
    ...
    public void ReferenceHandlingWithAdditionalParameters()
    {
        ...
        TestHelper
            ...
            .HaveMapMethodBody(
                """
                var refHandler = new global::DomainMap.Abstractions.ReferenceHandling.PreserveReferenceHandler();
    ...
    [Fact]
    public Task AdditionalParameterShouldNotLeakIntoNestedAutoGeneratedMapping()
    {
        var source = TestSourceBuilder.MapperWithBodyAndTypes(
            """
            [MapperIgnoreSource(nameof(A.Id))]
            partial B Map(A src, int id);
            """,
            """
            class A
            {
                ...
                public ANested Nested { get; set; }
            }
            class B
            {
                public int Id { get; set; }
                public string Name { get; set; }
                public BNested Nested { get; set; }
            }
            class ANested
            {
                public int Id { get; set; }
