...
namespace DomainMapper.Tests.Benchmarking;
...
public class ComparisonBenchmarkGateTest
{
    ...
    {
        var report = WriteReport(("MapperlyFlat", 100, 64), ("DomainMapperFlat", 120, 96));
    ...
    {
        var report = WriteReport(("MapperlyFlat", 100, 64), ("DomainMapperFlat", 126, 64));
    ...
    {
        var report = WriteReport(("MapperlyExistingTarget", 2, 0), ("DomainMapperExistingTarget", 2.9, 0));
    ...
    {
        var report = WriteReport(("MapperlyExistingTarget", 2, 0), ("DomainMapperExistingTarget", 3.1, 0));
    ...
    {
        var report = WriteReport(("MapperlyFlat", 100, 64), ("DomainMapperFlat", 100, 136));
    ...
    [Fact]
    public void FailsWhenDomainMapperPairIsMissing()
    {
        ...
        result.Passed.ShouldBeFalse();
        result.Errors.ShouldHaveSingleItem().ShouldContain("Missing DomainMapper benchmark pair");
    }

    [Fact]
    public void FailsWhenMapperlyPairIsMissing()
    {
        var report = WriteReport(("MapperlyFlat", 100, 64), ("DomainMapperFlat", 100, 64), ("DomainMapperUnpaired", 100, 64));
    ...
    {
        var mapperlyFirst = WriteReportWithValues(("MapperlyFlat", [10d, 11], 64), ("DomainMapperFlat", [12d, 13], 64));
        var domainMapperFirst = WriteReportWithValues(("MapperlyFlat", [12d, 13], 64), ("DomainMapperFlat", [10d, 11], 64));
        var options = new ComparisonGateOptions(1, 0, 1, 0)
        {
            ProvenParityScenarios = new HashSet<string>(["Flat"], StringComparer.Ordinal),
        };

        var result = ComparisonBenchmarkGate.Evaluate([mapperlyFirst, domainMapperFirst], options);
        ...
        comparison.DomainMapperSampleCount.ShouldBe(4);
        comparison.Expectation.ShouldBe("PROVEN PARITY");
        comparison.MapperlyMedianNanoseconds.ShouldBe(11.5);
        comparison.DomainMapperMedianNanoseconds.ShouldBe(11.5);
    }

    [Fact]
    public void RequiresEveryBalancedReportToContainEveryScenarioPair()
    {
        var completeReport = WriteReport(("MapperlyFlat", 10, 64), ("DomainMapperFlat", 9, 64));
    ...
    {
        var report = WriteReport(("MapperlyFlat", 100, 64), ("DomainMapperFlat", 200, 65));
        var options = new ComparisonGateOptions(1, 0, 1, 0)
        {
            ProvenParityScenarios = new HashSet<string>(["Flat"], StringComparer.Ordinal),
        };
    ...
    public void FasterExpectationRequiresOneSidedStatisticalWin()
    {
        var report = WriteReportWithValues(
            ...
            ("DomainMapperNestedCollection", [8d, 8, 8], 80)
        );
    ...
    public void FasterExpectationRejectsInconclusiveSamples()
    {
        var report = WriteReportWithValues(
            ("MapperlyNestedCollection", [10d, 10, 10], 100),
            ("DomainMapperNestedCollection", [9d, 10, 11], 100)
        );
        var options = new ComparisonGateOptions(1.25, 0, 1, 0)
        {
            RequireFasterScenarios = new HashSet<string>(["NestedCollection"], StringComparer.Ordinal),
    ...
    public void WildcardRequiresEveryDifferentiatedScenarioToBeFaster()
    {
        var report = WriteReportWithValues(
            ...
            ("DomainMapperFlat", [8d, 8, 8], 64),
            ("MapperlyNestedCollection", [20d, 20, 20], 128),
            ("DomainMapperNestedCollection", [21d, 21, 21], 128)
        );
        var options = new ComparisonGateOptions(1.25, 0, 1, 0)
        {
            ProvenParityScenarios = new HashSet<string>(["Flat"], StringComparer.Ordinal),
    ...
    {
        var report = WriteReportWithValues(("MapperlyFlat", [10d, 10, 10], 64), ("DomainMapperFlat", [8d, 8, 8], 64));
        var options = new ComparisonGateOptions(1.25, 0, 1, 0)
        {
            RequireFasterScenarios = new HashSet<string>(["*"], StringComparer.Ordinal),
    ...
    {
        var domainMapperGenerated = BuildGeneratedMapper("DomainMapperBenchmarkMapper", "MapLine(source)");
        var mapperlyGenerated = BuildGeneratedMapper("MapperlyBenchmarkMapper", "MapLine(source + 1)", placeExpression: "Create(source)");
        var declarations = """
            partial class DomainMapperBenchmarkMapper
            {
        ...
        var result = ComparisonCodeParity.Evaluate(domainMapperGenerated, mapperlyGenerated, declarations);
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"domainmapper-benchmark-{Guid.NewGuid():N}.json");
        var report = new
        {
