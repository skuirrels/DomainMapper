...
# pack a nupkg for each Roslyn version supported by DomainMap
...
echo "building DomainMap v${RELEASE_VERSION} for ${DOMAINMAP_ENVIRONMENT:-'local'}"
echo "cleaning artifacts dir"
mkdir -p "${artifacts_dir}"
rm -rf "${artifacts_dir:?}"/*

artifacts_dir="$(realpath "$artifacts_dir")"
source_generator_path="$(realpath "${script_dir}/../src/DomainMap")"
...
zipmerge "${artifacts_dir}/DomainMap.${RELEASE_VERSION}.nupkg" "${artifacts_dir}"/*/*.nupkg
echo "built ${artifacts_dir}/DomainMap.${RELEASE_VERSION}.nupkg"
